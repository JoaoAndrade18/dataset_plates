...
def load_infer_model1(inference_model_path: str, gpu: bool = True):
    ...
    try:
        ...
        model = torch.hub.load('ultralytics/yolov5', 'custom', path=inference_model_path, device=device, force_reload=True)
        model.eval()
        return model
    except Exception as e:
        print(f"[ERROR] Erro ao carregar modelo YOLOv5: {e}")
        return None
...
def upscale_image(image):
    ...
    try:
        ...
        print(f"[ERROR] Erro no upscaling: {e}")
        return image
...
def ocr_detect(frame, model, class_names):
    ...
    try:
        ...
        plate_list = []
        res = model(frame)
        print(f"[LOG] Detecções encontradas: {len(res.xyxy[0])}")
        if len(res.xyxy[0]) == 0:
            print("PROBLEMA: Nenhuma detecção encontrada!")
        for i in res.xyxy[0]:
            x1 = int(i[0])
            y1 = int(i[1])
            x2 = int(i[2])
...
def load_models(models: list[str], inference_model_path: str, class_names_path: str, gpu: bool = True, use_plate_detection: bool = True):
    ...
    if 'qwen' in models:
        ...
        try:
            ...
            print(f"[ERROR] Erro ao carregar Qwen: {e}")
    ...
    if 'yolov5' in models:
        ...
        try:
            ...
            print(f"[ERROR] Erro ao carregar class_names: {e}")
...
def execute_ocr_for_image(
    ...
    if use_plate_detection and model_dict['detect_plate']:
        ...
        if plate_image is not None:
            print(f"[INFO] Placa detectada: tamanho {plate_image.size}")
            if save_debug_images:
    ...
    if plate_image is not None:
        ...
        if use_preprocess:
            ...
            print(f"[INFO] Imagem após pré-processamento: tamanho {input_image.size}")
            if save_debug_images:
                input_image.save(os.path.join(debug_dir, f"{os.path.basename(image_path)}_preprocessed.png"))
    else:
        print(f"[INFO] Usando imagem inteira para {image_path}")
        input_image = preprocess_image(
            image_path,
            use_grayscale=use_grayscale,
            use_blur=use_blur,
    ...
    if model_dict['paddleocr']:
        ...
        try:
            ...
            texto = ' '.join([line for line in textos]) if textos else ''
            print(f"[INFO] PaddleOCR resultado para {image_path}: {texto}")
        except Exception as e:
            print(f"[ERROR] Erro no PaddleOCR para {image_path}: {e}")
            texto = ''
    ...
    if model_dict['pytesseract']:
        ...
        try:
            ...
            text = text.strip()
            print(f"[INFO] Tesseract resultado para {image_path}: {text}")
        except Exception as e:
    ...
    if model_dict['qwen_model']:
        ...
        try:
            ...
            if match:
                ...
                lines = [line.strip() for line in resultado_final.split('\n') if line.strip()]
                extracted_text = lines[-1] if lines else resultado_final
            
            placa_match = re.search(r'[A-Z0-9]{6,8}', extracted_text)
            final_result = placa_match.group() if placa_match else extracted_text
            print(f"[INFO] Qwen resultado para {image_path}: {final_result}")
        except Exception as e:
            print(f"Erro no Qwen para {image_path}: {e}")
            final_result = ''
        time_ms = (time.time() - start_time) * 1000
        correct = count_correct_letters(final_result, gold_plate)
        results['qwen_text'] = final_result
    ...
    if model_dict['yolo_model']:
        ...
        try:
            ...
            print(f"[INFO] YOLOv5 resultado para {image_path}: {text}")
        except Exception as e:
...
run_tests(
    ...
    models_list=["paddleOCR", "yolov5", "tesseract", "qwen"],
