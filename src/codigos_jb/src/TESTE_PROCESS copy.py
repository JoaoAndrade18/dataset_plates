...
def enhance_text(image):
    ...
    # kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3)) # acima de 2 da erro
...
def full_process(image):
    ...
    imgGray = cv2.cvtColor(processed.copy(), cv2.COLOR_BGR2GRAY)
    ...
    imgBgr = cv2.cvtColor(dilate.copy(), cv2.COLOR_GRAY2BGR)


    # cv2.imshow("Upscaled Image", processed)
    # cv2.waitKey(0)
...
def process_folder_with_full_process(folder_path):
    ...
    for filename in os.listdir(folder_path):
        # if filename.lower().endswith(('.jpg_plate.png', '.jpg_plate.jpg', '.jpg_plate.jpeg')) and not filename.startswith("processed_"):
        if filename.lower().endswith(('.png')) and filename.startswith("crop_"):
...
# test_full_process("frames_gold/frame_328.jpg_plate.jpg", save_result=True) 
# test_full_process("frames_gold/frame_244.jpg_plate.jpg", save_result=True)
process_folder_with_full_process("amostras_ufpr")
